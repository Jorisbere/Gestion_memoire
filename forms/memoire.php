...
while ($row = $encadrant_result->fetch_assoc()) { $encadrants[] = $row['username']; }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($titre) || empty($annee) || empty($fichier['name'])) {
        ...
        if (move_uploaded_file($fichier['tmp_name'], $targetPath)) {
           $stmt = $conn->prepare("INSERT INTO memoires (user_id, titre, annee, fichier_path, date_depot, encadrant) VALUES (?, ?, ?, ?, NOW(), ?)");
           $stmt->bind_param("issss", $user_id, $titre, $annee, $targetPath, $encadrant);
           if ($stmt->execute()) { $success = "Mémoire déposé avec succès."; } else { $error = "Erreur lors de l'enregistrement."; }
           $stmt->close();
...
<head>
  ...
  <script>
function confirmerDepot() { return confirm("Êtes-vous sûr de vouloir déposer ce mémoire ?"); }
</script>
  <style>
    ...
    .back-button a { text-decoration: none; color: #0078D7; font-weight: bold; font-size: 16px; transition: color 0.3s ease; }
    .back-button a:hover { color: #0056a3; }
    ...
    h2 { text-align: center; margin-bottom: 25px; font-size: 24px; color: #0078D7; }
    label { display: block; margin-top: 15px; font-weight: 600; color: #333; }
    input[type="text"], input[type="file"], select { width: 100%; padding: 12px; margin-top: 8px; border-radius: 10px; border: 1px solid #ccc; font-size: 15px; background:rgb(229, 232, 232); transition: border-color 0.3s ease; }
    input:focus { border-color: #0078D7; outline: none; }
    .button-container { display: flex; justify-content: center; margin-top: 12px; padding: 12px; }
    button { padding: 14px 24px; background: #0078D7; color: #fff; border: none; border-radius: 10px; cursor: pointer; font-size: 16px; transition: background 0.3s ease; }
